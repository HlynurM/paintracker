...
The primary user action: log a headache event by selecting a severity (1–10) and optionally adding notes or trigger tags. The current weather snapshot is captured automatically at submission time so the entry carries the conditions at that exact moment. The entry is persisted to local storage immediately.

---

## Acceptance Criteria

- [ ] A headache can be logged by selecting a severity between 1 and 10.
...
- [ ] After a successful log, the form resets to its defaults (severity 5, notes cleared, triggers cleared).
- [ ] The submit button is disabled while a save is in progress, preventing double-submission.
- [ ] If weather data is unavailable, the entry is still saved (weather field is left empty — user is not blocked).
...
1. The form collects:
   - `severity` — required, integer 1–10, presented as a slider
...
5. `headacheService.logHeadache(formData)` validates that `severity` is in range 1–10 before saving; throws on invalid input.
6. Duplicate submission within 1 second of the last save is silently rejected (debounce guard in the service).
7. On successful save, the form resets to its default state (severity defaults to 5, notes cleared, triggers cleared).
...
|---|---|---|---|
| `severity` | `HeadacheSeverity` (1–10) | Yes | |
...
- `weatherStore.currentWeather` is null → entry is saved with null weather; no error is thrown or shown to the user.
- User taps submit twice rapidly (within 1 second) → second call is silently dropped; only one entry is saved.
- Severity slider at boundary values (1 and 10) → both must submit successfully.
...
- **Key scenarios — `headacheService`:**
  ...
  - Severity `0` or `11` → throws a validation error; nothing is saved
...
- **Key scenarios — `HeadacheForm` component:**
  - Slider defaults to 5 on mount
  - Submitting fires `logHeadache` with the correct severity value
  - After successful submit, severity resets to 5 and notes field clears
